...
| Dizionari / mappe native          | Array associativi (bash 4+) | Hash | No | Record strutturati | No | `map(...)` typed-map; `Dict()` immutabile con API chiara (`set`, `get`, `has`, `delete`, `keys`, `values`, bridge JSON); `declare -A` / `typeset -A` espongono array associativi shell sopra `Dict`; `Matrix(col...)` tabella bidimensionale con `add_row`, `select`, `where`, `to_csv`, `to_json` (E6-S8) |
